...
Starte JETZT mit Schritt 1!"""
        ...
        if has_existing_metadata and has_existing_types:
            ...
            # Mode B: No apps yet - use auto-persist workflow
            query = """🚀 AUFGABE: Erstelle ein neues Dashboard MIT Datenpersistenz!

Du hast KEINE existierenden LivingApps. Nutze den auto-persist Workflow:

PHASE 1 - UI DESIGN:
1. ⚠️ ZUERST: Aktiviere frontend-design Skill → erstelle design_brief.md
2. Lies design_brief.md und verstehe die UI-Struktur
3. Baue Dashboard.tsx mit MOCK-DATEN (folge dem Design Brief)

PHASE 2 - LIVINGAPPS ERSTELLEN:
4. Rufe mcp__dashboard_tools__create_apps auf mit den benötigten Apps
   Beispiel: {"apps": [{"name": "Tasks", "identifier": "tasks", "controls": {...}}]}
5. Rufe mcp__dashboard_tools__generate_typescript auf mit der zurückgegebenen metadata

PHASE 3 - UI VERKNÜPFEN:
6. Ersetze Mock-Daten durch echte API-Calls (LivingAppsService)
7. Füge Loading-States hinzu
8. Teste mit 'npm run build'

PHASE 4 - DEPLOY:
9. Rufe mcp__dashboard_tools__deploy_to_github auf

⚠️ WICHTIG:
- NIEMALS Mock-Daten im finalen Dashboard lassen!
- IMMER LivingApps erstellen wenn Daten gespeichert werden müssen!
- Siehe .claude/skills/auto-persist/SKILL.md für Details!

Starte JETZT mit Phase 1!"""
            print(f"[LILO] Build-Mode: Neues Dashboard mit Auto-Persist")
